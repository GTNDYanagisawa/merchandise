...
class VoucherTests extends BaseWSTest {
	...
	def getVoucher(code, params) {
		def client_credentials_token = testUtil.getTrustedClientCredentialsToken()
		def con = testUtil.getSecureConnection("/vouchers/${code}?${params}", 'GET', 'JSON', HttpURLConnection.HTTP_OK, null, null, client_credentials_token)
	...
	@Test
	void testGetVoucherBasicFields() {
		def response = getVoucher(PROMOTION_VOUCHER_CODE, 'fields=BASIC')
		...
		assert !response.valueString
		assert !response.valueFormatted
		assert !response.freeShipping
	}

	@Test
	void testGetVoucherFullFields() {
		def response = getVoucher(PROMOTION_VOUCHER_CODE, 'fields=FULL')
		assert response.code == PROMOTION_V_CODE
		assert response.voucherCode == PROMOTION_VOUCHER_CODE
	...
	@Test
	void testGetVoucherAbsoluteBasicFields() {
		def response = getVoucher(ABSOLUTE_VOUCHER_CODE, 'fields=BASIC')
	...
	@Test
	void testGetVoucherAbsoluteFullFields() {
		def response = getVoucher(ABSOLUTE_VOUCHER_CODE, 'fields=FULL')
		assert response.code == ABSOLUTE_V_CODE
		assert response.voucherCode == ABSOLUTE_VOUCHER_CODE
